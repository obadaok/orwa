...
هـ. صلاحية الجدولة الدقيقة (SCHEDULE_EXACT_ALARM) والمنبه الدقيق (USE_EXACT_ALARM)
- لماذا نحتاجها: لضمان وصول تذكيرات الأذكار في الوقت المحدد بدقة (مطلوبة في Android 12+).
- كيف نستخدمها: فقط للتذكيرات التي يضبطها المستخدم بوقت محدد.
- لا نستخدمها: لأي غرض آخر غير التذكيرات.

و. صلاحية التخزين (READ/WRITE_EXTERNAL_STORAGE) — للإصدارات القديمة فقط
- لماذا نحتاجها: لحفظ صور الاقتباسات التي ينشئها المستخدم في مجلد الصور بالجهاز.
- كيف نستخدمها: فقط عند الضغط على "حفظ" في محرر الاقتباسات. في الإصدارات الحديثة (Android 10+) نستخدم MediaStore بدون هذه الصلاحية.
- لا نستخدمها: للوصول إلى ملفاتك الشخصية أو قراءة أي بيانات غير متعلقة بالتطبيق.

ز. صلاحيات أخرى (RECEIVE_BOOT_COMPLETED، FOREGROUND_SERVICE، WAKE_LOCK، BLUETOOTH)
...
- BLUETOOTH وBLUETOOTH_CONNECT: للتحكم في نقل الصوت إلى الأجهزة المتصلة.
